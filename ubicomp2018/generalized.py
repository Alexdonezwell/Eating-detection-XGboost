...
for subj in subj_list:
    ...
    std_scale = preprocessing.StandardScaler().fit(X_train_generalized)
    normalized_train = std_scale.transform(X_train_generalized)
    ...
    for fold in range(14):
        ...
        dtrain = xgb.DMatrix(normalized_train, label=Y_train_generalized)
        ...
        df_results.to_csv(os.path.join(generalized_folder, "prediction_day_{}.csv".format(fold)))
        ...
        np.savetxt(os.path.join(generalized_folder, "day_{}.csv".format(fold)), cm, fmt='%d')
    ...
    np.savetxt(os.path.join(generalized_folder, "cm_total.csv".format(fold)), cm_total, fmt='%d')
    ...
    df_result.to_csv(os.path.join(generalized_folder, '{}.csv'.format(subj)))
    ...
    with open(os.path.join(generalized_folder, 'latex.txt'), 'w') as f:
        f.write(latex)
